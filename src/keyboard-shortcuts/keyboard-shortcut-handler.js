...
export default function KeyboardShortcutHandler(options) {
	...
	let sequenceDelay = options.sequenceDelay || DEFAULT_SEQUENCE_DELAY;
	let keyBindings = options.keyBindings;
	...
	const addCommandHandlers = (commandHandlers) => {
		commandHandlers.forEach(({ context, command, handler }) => {
			let key = getContextCommandKey(context, command);
	 ...
	 */
	const getCommandHandlers = ({ context, command }) => handlers[getContextCommandKey(context, command)] || [];
	...
	const processCommands = (commands, e) => {
		...
		let handled = false, allContextCommand;
		...
		!handled && allContextCommand && getCommandHandlers(allContextCommand).forEach(handler => {
			handled=true;
	...
	const handleKeyDown = ({ e }) => {
		...
		if (target.nodeName.match(/INPUT|TEXTAREA|SELECT|OPTION/) && !e.disabled || target.isContentEditable ||
			e.key.match(/Control|Alt|Shift|Meta/)) return;
		...
		let key = [e.metaKey && 'cmd', e.ctrlKey && 'ctrl', e.shiftKey && 'shift', e.altKey && 'alt', e.key.toUpperCase()].filter(Boolean).join('+');
		...
		let { commands, sequences } = keyBindings.getCommandsAndSequences(key);
	 ...
	 */
	const getShortcuts = () => {
		let bindings = keyBindings.getBindings();
		return Object.keys(bindings).reduce((result, context) => {
			let shortcutsForContext = flatten(bindings[context], context);
	...
	const flatten = (root, context, compoundKey) => {
		...
		return Object.keys(root).reduce((result, key) => {
			let subResult = flatten(root[key], context, compoundKey ? `${compoundKey} ${key}` : key);
