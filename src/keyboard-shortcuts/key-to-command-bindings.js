...
export default function KeyToCommandBindings(initialBindings=DEFAULT_BINDINGS) {
	...
	*/
	const getCommandsAndSequences = (key) => Object.keys(bindings).reduce((result, context) => {
		let command = get(bindings[context], key);
		if (command) {
			typeof command === 'object' ? result.sequences.push({ context, command: command.default }) : result.commands.push({ context, command });
		}
		return result;
	}, { commands: [], sequences: [] });
	 ...
	 */
	const replaceBindings = (newBindings) => {	bindings = { ...newBindings }; };
