...
export function toggleTaskDone({ ...task }) {
	[task.status, task.percentComplete] = isTaskDone(task) ? [NEED_STATUS, '0'] : [COMPLETE_STATUS, '100'];
	return task;
}
...
export function setDueDate(task, dueDate) {
	if (!getDueDate(task)) { return task; }
	return {
		...task,
		instances: [
...
export function sortByDueDate(taskA, taskB) {
	let aDue = getDueDate(taskA) || Number.POSITIVE_INFINITY;
	let bDue = getDueDate(taskB) || Number.POSITIVE_INFINITY;
