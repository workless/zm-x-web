	...
	try {
		while ((match = timeReg.exec(str)) != null) {
			jsonResult = {
				...
				[match[1]]: {
					enabled: match[2] === 'Y',
					start: moment(match[3], 'HHmm').format('h:mm A'),
					end: moment(match[4], 'HHmm').format('h:mm A')
				}
			};
		}
	...
	try {
		...
		Object.keys(jsonTime).forEach((dayNumber, index) => {
			soapTime = soapTime.concat(
				`${dayNumber}:${jsonTime[dayNumber].enabled
					? 'Y'
					: 'N'}:${switchTimeFormat(
					jsonTime[dayNumber].start
				)}:${switchTimeFormat(jsonTime[dayNumber].end)}${index !==
				Object.keys(jsonTime).length - 1
					? ','
					: ''}`
			);
		});
