...
export default class ProgressIndicator extends Component {
	...
	onResponse = () => {
		let requests = Math.max(0, this.state.requests-1);
		let id = ++this.counter;
		if (requests) {
	...
	increment = () => {
		this.progressBar.style.transform = this.progressBar.style.transform.replace(/(\d+)/, (s, d) => parseFloat(d)+1 );
	};

	componentDidMount() {
		this.props.zimbra.on('req', this.onRequest);
		this.props.zimbra.on('res', this.onResponse);
		this.props.link.on('req', this.onRequest);
		this.props.link.on('res', this.onResponse);
	}

	componentDidUpdate(prevProps, prevState) {
		let enabled = this.state.requests && !this.state.done,
	...
	render({ }, { requests, totalRequests, enabled, done }) {
		return (
			<div class={cx(style.progress, enabled && requests && style.enabled, done && style.done)} data-value={requests}>
				<div
					ref={this.progressRef}
					class={style.inner}
					style={{
						transform: `translateX(${done ? 100 : requests ? ((1 + totalRequests - requests)/(1 + totalRequests)*100) : 0}%)`
					}}
				/>
			</div>
		);
