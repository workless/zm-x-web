...
import { getJobDescription } from '../../utils/contacts';
...
function getDataPointsForRange(frequencyData, selectedRange) {
	return get(
		find(frequencyData, ['by', RANGES[selectedRange]]),
		'dataPoint',
		[]
	);
}
...
const ContactFrequencyCard = ({
	contact,
	contact: { attributes, attributes: { jobTitle, email, phone } },
	...
	const name = displayName(attributes);
	const jobDescription = getJobDescription(attributes);
	const dataPoints = getDataPointsForRange(
		contactFrequencyData,
		selectedRange
	).map(point => ({
		...point,
			...
			<div class={cx(s.section, s.pb0)}>
				<div class={s.topSection}>
					...
					<div class={s.topSectionAvatar}>
						<Avatar class={s.avatar} email={email} />
					</div>
				</div>
				<ContactCardMenu
