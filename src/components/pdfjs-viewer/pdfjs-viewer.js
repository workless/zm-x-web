...
export default class PdfjsViewer extends Component {
	...
	reset = () => {
		this._renderedPages = {};
		if (this.state.numPages !== 0) { this.setState({ ...PdfjsViewer.initialState }); }
	}

	getScaledViewport = (src, scale, pageIndex) => (
		getPDFJSDocument(src).then((pdfDoc) => pdfDoc.getPage(+pageIndex)).then((pdfPage) => {
			const canvas = this.refs && this.refs['canvas_' + pageIndex];
			if (!canvas) { return Promise.reject(`canvas ${pageIndex} does not exist`); }
			const fullScaleViewport = pdfPage.getViewport(1.0);
			return pdfPage.getViewport((canvas.parentNode.clientWidth / fullScaleViewport.width).toFixed(2) * .9 * scale);
		})
	)
	...
	getDocumentPagesViewports = (src, scale) => (
		getPDFJSDocument(src).then((pdfDoc) => {
			...
			return Promise.all(queue).then((scaledViewports) => scaledViewports.reduce((acc, scaledViewport, index) => ({
				...acc,
	...
	handleScaleDocument = ({ src, scale, overscan, ...props }) => {
		...
		this.getDocumentPagesViewports(src, scale).then((pageViewportMap) => {
			...
			if (this.state.currentPage) {
				this.renderPageRange({ start: this.state.currentPage - overscan, end: this.state.currentPage + overscan, src, scale, ...props });
	...
	handleScrollContainer = throttle(({ target }) => {
		...
		if (!elementAtCentroid || elementAtCentroid.tagName !== 'CANVAS') { return; }

		const currentPage = Array.prototype.indexOf.call(elementAtCentroid.parentNode.childNodes, elementAtCentroid) + 1;
	...
	componentDidMount() {
		if (this.props.container) {
			this.props.container.addEventListener('scroll', this.handleScrollContainer, { passive: true });
		}
	...
	componentWillReceiveProps({ src, scale, container, ...nextProps }) {
		...
		if (this.props.container !== container) {
			this.props.container && this.props.container.removeEventListener('scroll', this.handleScrollContainer, { passive: true });
			container && container.addEventListener('scroll', this.handleScrollContainer, { passive: true });
		}
	...
	componentWillUnmount() {
		if (this.props.container) {
			this.props.container.removeEventListener('scroll', this.handleScrollContainer, { passive: true });
		}
	}

	renderPage = ({ pageIndex, src, scale, onLoadPage }) => this._renderedPages[pageIndex] || (
		this._renderedPages[pageIndex] = getPDFJSDocument(src).then((pdfDoc) => pdfDoc.getPage(+pageIndex))
			.then((pdfPage) => {
				const canvas = this.refs && this.refs['canvas_' + pageIndex];

				if (!canvas) { return Promise.reject('canvas does not exist'); }

				return this.getScaledViewport(src, scale, pageIndex).then((scaledViewport) => (
					pdfPage.render({
	...
	renderPageRange = ({ start, end, ...rest }) => {
		for (let pageIndex = Math.max(1, start); pageIndex <= Math.min(this.state.numPages, end); ++pageIndex) {
	...
	render({ src, scale, overscan, onLoad, container, ...props }, { numPages, pageViewportMap }) {
		return (
			<div class={cx(style.container, props.class)}>
				{Array(...Array(numPages)).reduce((acc, _, index) => ([
					...acc,
					<canvas
						{...props}
