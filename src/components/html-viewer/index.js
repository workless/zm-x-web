...
export default class HtmlViewer extends PureComponent {
	...
	resize = () => {
		...
		let scale = String(this.props.scale)==='true',
	...
	render({ html, scale, fit, ...props }, { scalingRatio=1 }) {
		...
		return (
			<div data-css-reset={resetId} {...props} class={cx(props.class, style.htmlViewer)}>
				<div
					...
					dangerouslySetInnerHTML={{ __html: sanitize(html, PROCESSORS) }} // eslint-disable-line react/no-danger
