...
export default class KeyboardShortcutsModal extends Component {
	...
	render({ visible, hide },state, { shortcutCommandHandler }) {
		...
		let shortcuts = shortcutCommandHandler.getShortcuts();
		return (
			<div>
				<ModalDialog title="Keyboard Shortcuts" onClose={hide} buttons={false} cancelButton={false} >
					<div>
		...
		<ul class={style.commandListWrapper}  >
			{ commands.map(([shortcut, displayName]) => (
				<li>
					...
					<span class={style.displayName}><Text id={`shortcuts.${displayName}`}>{displayName}</Text></span>
				</li>
			))
			}
		</ul>
