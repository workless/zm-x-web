...
export default class AttachmentsPane extends Component {
	...
	render = ({
		...
		const allAttachments =
			items &&
			items.reduce((old, current) => [].concat(old, current.attachment), []);
		const listProps = {
			attachItems,
			allAttachments,
			onDownload,
		...
		return (
			<div
				class={cx(
					style.attachmentPane,
					isAttachmentViewerOpen && style.attachmentViewerOpen
				)}
			>
	...
	<div class={style.attachmentList} scrollbar>
		{attachItems.map(([label, itemsByDate]) => (
			<div key={label} class={style.messageGroup}>
				{label && (
					<div class={cx(style.dateRowLabel, style.wideDateRowLabel)}>
						{label}
					</div>
				)}
