	...
	render({ grant }) {
		...
		return (
			<tr class={s.accessTableRow}>
				...
				<td>
					<CloseButton aria-label="Remove" onClick={this.handleRemoveGrant} />
				</td>
			</tr>
		);
	}
...
const ShareAccessSettings = ({
	...
	<div>
		<div class={s.sectionTitle}><Text id="calendar.dialogs.share.accessSection" /></div>
		<div class={s.section}>
			<table class={s.accessTable}>
				{emailGrants.length ? emailGrants.map(g => (
					<GrantRow
						grant={g}
						onRemoveEmailGrant={onRemoveEmailGrant}
						onUpdateGrant={onUpdateGrant}
					/>
				)) : <tr><td><Text id="calendar.dialogs.share.noAccess" /></td></tr>}
			</table>
		</div>
		<div>
			<div class={s.sectionTitle}><Text id="calendar.dialogs.share.inviteEmailsSection" /></div>
			<EmailShareInfoCard
				aclEmails={aclEmails}
