...
export default class AppointmentPopover extends Component {
	render({
		...
		return (
			<div>
				<Button styleType="floating" class={s.closeButton} onClick={onClose} />
				<div class={s.container}>
					<div class={s.mainInfo}>
						...
						{start && (
							<div class={cx(s.infoLine, s.alignCenter)}>
								...
								{item.alarm === true && (
									<Icon class={s.alarm} name="fa:bell" size="xs" />
								)}
								{searchResult.recur &&
									<div class={s.repeatIcon} />
								}
							</div>
						)}
					</div>
					<div class={s.moreInfo}>
						...
						{excerpt && (
							<div class={cx(s.infoLine, s.eventDesc)}>{excerpt}</div>
						)}
					</div>
