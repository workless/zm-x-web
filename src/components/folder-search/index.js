	...
	render(props) {
		...
		return (
			<div>
				...
				{props.folder.folders && props.folder.folders.map(f => (
					<Folder
						{...props}
						folder={f}
						depth={props.depth + 1}
...
const SearchFolderGroups = ({ onAllMail, folderGroups, specialFolders, customFolders, groupClass, ...rest }) => (
	<div>
		...
		<ActionMenuGroup>
			<div
				class={cx(s.searchFolderGroup, groupClass)}
			>
					...
					{specialFolders.map(f => (
						<div className={s.specialFolder}>
							<Folder
								...
								depth={1}
								folder={f}
								selected={rest.keyboardSelection === f.id}
							/>
						</div>
					))}
				</div>
				<div>
					<p class={cx(s.title)}>Folders</p>
					{customFolders.map(f => (
		...
		<ActionMenuGroup>
			<ActionMenuItem className={s.advanceSearch}>
				<div>
...
function selectOffsetFolder(folderGroups, currentId, offset = 1) {
	...
	if (index === -1 && offset >= 1) { return folders[0].id; }
	if (index === 0 && offset <= -1) { return currentId; }
...
export class FolderSearch extends Component {
	...
	handleFilterChange = (e, folders) => {
		...
		const newSelection = value === ''
			? null
			: selectOffsetFolder(newFolderGroups, null, 1);
	...
	handleKeyboardEnter = () => {
		if (!this.state.keyboardSelection) { return; }
	...
	render({
		...
		const folderGroups = filterValue !== ''
			? [filteredFolders(folders, filterValue)]
			: rest.folderGroups;
