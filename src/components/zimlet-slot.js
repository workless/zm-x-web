...
export default class ZimletSlot extends Component {
	...
	getChildren(name, props) {
		let results = this.context.zimlets.invokePlugin('slot::' + name, props);
		if (results) {
	...
	render({ name, props, children, ...allProps }, state, { zimlets }) {
		let results = this.getChildren(name, props || allProps);
		...
		return (<span {...(props ? allProps : {})}>
			{ zimlets.showZimletSlots &&
				<span style="position: absolute; background: rgba(228,147,51,.5); border: 2px solid red; padding: 2px; font-weight: 600; font-size: 12px;">
