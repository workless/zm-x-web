...
export default class AddressGroup extends Component {
	...
	componentWillReceiveProps(props) {
		let { prefix, suffix } = getAddressFieldPrefixAndSuffix(props.attribute);
		this.setState({
			prefix,
			suffix
		});
			...
			<div class={style.addressGroup}>
				{
					ADDRESS_FIELDS.map(field => {
						let FieldComponent = FIELD_MAPS[field] || 'input';
						let fieldKey = prefix + field + suffix;
						return (
							readonly ? (
								...
								<div class={cx(style.inputWrap, style.inputDropdown)}>
									<Localizer>
										<Select
											iconPosition="right"
											anchor="left"
											displayValue={contact.attributes[fieldKey] || <Text id={'contacts.edit.fields.Country'} />}
