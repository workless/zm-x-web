...
@connect(null, bindActionCreators.bind(null, contactsActionCreators))
...
export default class ImportContactsFromFile extends Component {
	...
	import = () => {
		let { importContacts, setLastUpdated } = this.props,
		...
		return (
			...
			>
				<form onSubmit={this.import} action="javascript:">
					<p class={style.description}><Text id="contacts.importFromFile.DESCRIPTION" /></p>

					{ error && (
						<div key="error" class={style.error}>
							Error: <Text id={error}>{error}</Text>
						</div>
					) }

					<input type="file" accept=".csv, text/csv, .vcf, text/vcard" onChange={this.setFile} ref={this.fileRef} />
				</form>
			</ModalDialog>
		);
	}
