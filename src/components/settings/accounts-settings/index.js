...
class AccountsSettings extends Component {
	...
	componentWillUpdate(nextProps) {
		if (
			!nextProps.value.showMailForwardingAddress &&
			this.state.showMailForwardingAddress
		) {
			this.setState({
				showMailForwardingAddress: true
			});
	...
	render(
		...
		const selectedAccount =
			selectedAccountId && find(accounts, ['id', selectedAccountId]);
		...
		const ConfirmDeleteProps = {
			...
			accountName:
				find(accounts, account => account.id === selectedAccountId) &&
				find(accounts, account => account.id === selectedAccountId).name,
			switchView: this.switchActiveScreen,
			accountInfoQuery
		};
