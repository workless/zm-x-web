...
export default class TaskItem extends Component {
	...
	render({ task, folders, hidePriority, onShowEdit, onDelete, onChange }, { name, edit, isDone }) {
					...
					<TaskPopover task={task} folders={folders} onDelete={onDelete} onEdit={onShowEdit}>
						<div>
							<div class={cx(style.firstLine, edit && style.isEditing)}>
								<input class={style.checkbox} type="checkbox" checked={isDone} onClick={this.handleCheckboxClick} />
								{ edit &&
								...
								{ !edit &&
									<span class={cx(style.text, isDone && style.isDone)} onDblClick={this.toggleEdit} title={task.name}>{task.name}</span>
								}
								{ !edit && !hidePriority &&
									task.priority === URGENT_PRIORITY ? '!!' : task.priority === IMPORTANT_PRIORITY ? '!' : ''
								}
							</div>
							<div class={style.dueDate}>
								{ dueDate }
							</div>
						</div>
					</TaskPopover>
