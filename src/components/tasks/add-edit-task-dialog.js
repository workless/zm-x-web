...
import { URGENT_PRIORITY, IMPORTANT_PRIORITY, NORMAL_PRIORITY, NEED_STATUS } from '../../constants/tasks';
...
@graphql(TaskQuery, {
	...
	props: ({ data: { loading, task } }) => (loading ? { loading } : {
...
export default class AddEditTaskDialog extends Component {
	...
	calcState = ({ folders, folderId, task: { name, priority, status, inviteId, ...task }, notes }) => ({
		dueDate: getDueDate(task),
		folderId: folderId || folders[0].id,
		name,
		priority: parseInt(priority, 10) || NORMAL_PRIORITY,
	...
	confirm = () => {
		const { onSave, task: { inviteId } } = this.props;
	...
	render({ folders, task, loading, onClose }, { dueDate, folderId, name, notes, priority }) {
		const { inviteId } = task;
			...
			>
				<AlignedForm>
					...
					<FormGroup>
						...
						<DateInput
							dateValue={dueDate}
							onDateChange={linkState(this, 'dueDate')}
						/>
					</FormGroup>
					<FormGroup>
						<AlignedLabel>
						...
						</AlignedLabel>
						{
							...
							>
								...
								<option value={NORMAL_PRIORITY} key={`priority-normal`}><Text id="tasks.normal" /></option>
								<option value={IMPORTANT_PRIORITY} key={`priority-important`}><Text id="tasks.important" /></option>
								<option value={URGENT_PRIORITY} key={`priority-urgent`}><Text id="tasks.urgent" /></option>
					...
					<FormGroup>
						...
						<Select
							value={folderId}
							onChange={linkState(this, 'folderId')}
						>
