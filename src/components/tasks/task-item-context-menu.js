...
export default function TaskItemContextMenu({ children, onEdit, onDelete, onChange, onToggleDone, disabled, task }) {
	return  (
	...
	render({ onEdit, onDelete, onChange, onToggleDone, task }) {
		...
		return (
			<ActionMenuGroup>
				...
				<ActionMenuItem onClick={callWith(onChange, postponedTask)} disabled={!getDueDate(task) || isDone}>
