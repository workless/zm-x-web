...
export default class TaskGroup extends Component {
	...
	handleBeginRename = () => this.setState({ isRenaming: true, renamingInputValue: this.props.title });
	...
	handleRenameSubmit = () => {
		...
		renameTaskList({
			...
			.catch(({ message }) => {
				if (message) {
					this.props.notify({
						message: message.match(/object with that name already exists/) ? <Text id="tasks.duplicateListError" /> : message,
						failure: true
					});
				}
			});
	...
	add = () => {
		let { quickAddTaskName } = this.state;
		let { createTask, folderId, dueDate, priority } = this.props;
	...
	handleSaveTask = (task) => {
		...
		let saveMethod = task.inviteId ? this.props.onChange : this.props.createTask;
	...
	render({
		tasks, title, titleKey, hidePriority, hideAdd, onChange, folders, folderId, dueDate, priority, immutable, onDelete
	}, {
		isRenaming, renamingInputValue, quickAddTaskName, isCollapsed, showAddInput, hovering, showAddEditTaskDialog, editId
	}) {
		let heading = titleKey ? this.props[`${titleKey}Label`] : title ;
		if (heading) heading = heading.toUpperCase();
		return (
			<div
				class={cx(style.taskList, isCollapsed && style.collapsed, !isCollapsed && (hovering || showAddInput || quickAddTaskName) && style.isShowingAdd)}
				onMouseOver={this.handleMouseOver}
				onMouseOut={this.handleMouseOut}
			>
				{heading &&
					<header class={style.listHeader}>
						<ActionButton class={style.toggle} monotone icon="angle-down" onClick={this.toggleCollapsed} />
						{ isRenaming ? (
							<FolderInput
							...
							>
								<h5 title={heading} onDblClick={this.handleBeginRename}>{heading}</h5>
							</TaskListContextMenu>
						)}
						{!hideAdd && !isRenaming &&
							<ActionButton class={style.toggleAdd} monotone icon="plus" onClick={callWith(this.handleShowAddEditTaskDialog)} />
						}
					</header>
				}

				{ showAddEditTaskDialog &&
					<AddEditTaskDialog task={find(tasks, (task) => task.inviteId === editId)} folders={folders} folderId={folderId} dueDate={dueDate}
						priority={priority} onSave={this.handleSaveTask} onClose={this.hideAddEditTaskDialog} onDelete={onDelete}
					/>
				}
						...
						{!hideAdd &&
							<div class={cx(style.add, style.task, style.isEditing)}>
								<div class={style.firstLine}>
									<input class={style.checkbox} type="checkbox" disabled />
