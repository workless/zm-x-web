...
export class EmojiMenu extends Component {
	...
	renderEmojiRow = (emojiRow) => (
		<div class={style.emojiRow}>
			{ emojiRow.map( emoji => <Emoji emoji={emoji} onClick={this.addEmoji} /> ) }
		</div>
	);
					...
					<Tabs class={style.tabs} tabActiveClass={style.tabActive} >
						{
							emojiTabs.map(emojiTabItem => {
								let emojis = emojiGroups[emojiTabItem.name];
								return (
									<Tab
										title={<img class={style[emojiTabItem.name]} src={generateEmojiUrl(emojiTabItem.unicode)} alt={emojiTabItem.caption} title={emojiTabItem.caption} />}
...
export function getEmojiPropsFromShortName(shortname) {
	let unicodeEmoji = emojione.shortnameToUnicode(shortname);
	if (unicodeEmoji === shortname) return;
	return {
		name: shortname,
		url: generateEmojiUrl(String(unicodeEmoji).codePointAt(0).toString(16))
	};
}
