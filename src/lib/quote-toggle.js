...
export default function quoteToggle(document) {
	let all = find(document, '#zwchr, .gmail_quote, .gmail_extra, .yahoo-quoted-begin, #OLK_SRC_BODY_SECTION, body > blockquote, body > * > blockquote'),
		first = document.body.firstElementChild;

	if (first) {
		let hr = find(document, 'body > hr, body > '+(first.nodeName || 'div')+' > hr');
	...
	let rootNodes = find(document, '[style]');
	for (let i=0; i<rootNodes.length; i++) {
		let el = rootNodes[i];
		if (el.style.borderTop && el.style.borderTop.match(/\bsolid\b/i) && el.textContent.match(/^[\n\s]*From:/g)) {
	...
	let quoted = all.sort( (a, b) =>
		a.compareDocumentPosition(b) & 2 ? 1 : -1
	)[0];

	if (quoted) {
		// Also hide "lead-in" ("On May 5th, so and so wrote:")
		let prev = quoted.previousSibling;
		if (prev && prev.textContent.match(/^(\s*On \w.*?\sat\s.*?wrote:\s*$|\s*From: [\s\S]+\n\s*Sent: )/i)) {
			quoted = prev;
		}
