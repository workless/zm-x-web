...
/** Convert "\\uD83D\\uDC4B" to "\uD83D\uDC4B" */
const replaceSurrogatePair = (s, a, b) => String.fromCharCode(parseInt(a,16)) + String.fromCharCode(parseInt(b,16));
 ...
 */
export function getEmailHTMLDocument({ id, html, text, excerpt, attachments, inlineAttachments }, options) {
	options = options || {};
	...
	if (!html) {
		...
		if (text) {
			sanitized = '<body style="white-space:pre-wrap;">' + text.replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br />') + '</body>';
		}
	}
	else {
		let opts = {
...
function getAttachmentByCid(cid, attachments) {
	if (cid) {
		cid = String(cid).toLowerCase().replace(/^\s*?cid:\s*?/gi, '');
		for (let i=attachments.length; i--; ) {
			let { contentId, part, messageId } = attachments[i];
			if (String(contentId).toLowerCase().replace(/[<>]/g,'')===cid) {
				// @TODO use drop fallback
				return attachments[i].url || `/@zimbra/service/home/~/?auth=co&id=${encodeURIComponent(messageId)}&part=${encodeURIComponent(part)}`;
			}
		}
	}
	return false;
...
function sanitizeCss(css, attachments) {
	...
	css = css.replace(/url\(\s*?(['"]?)\s*?cid:\s*?(.*?)\s*?\1\s*?\)/gi, (s, b, cid) => `url("${getAttachmentByCid(cid, attachments) || ''}")`);
	return css;
}
...
function walk(node, opts) {
	...
	let n = String(node.nodeName).toLowerCase();
	...
	if (node.attributes) {
		...
		for (let i=attrs.length; i--; ) {
			...
			if (lcName.match(URL_ATTRIBUTES)) {
				let matches = String(value).match(/^\s*?cid\s*?:\s*(.+?)\s*$/i);
				if (matches) {
					node.setAttribute('data-cid', matches[1]);
					let attachment = getAttachmentByCid(matches[1], att) || '';
			...
			if (lcName.match(JS_ATTRIBUTES)) {
				let matches = String(value).match(/^\s*?javascript\s*?:\s*?(.*)$/i);
...
export function insertAtCaret(win=window, html, rootElement) {
	...
	if (!range || (rootElement && !(rootElement===range.startContainer || rootElement.contains(range.startContainer)))) {
...
export function replaceWordBeforeCursor(predicateFn, onReplace) {
	...
	let content = sel.anchorNode.textContent;
	let token = content.slice(0, sel.anchorOffset).split(/\s/).pop();
	let predicateResponse = predicateFn(token);
...
export function moveSelectionOutOfNonEditableArea(rootElement) {
	let sel = window.getSelection(),
		range = sel && sel.getRangeAt && sel.rangeCount!==0 && sel.getRangeAt(0),
		ancestor = range && range.commonAncestorContainer;

	if (!ancestor || !rootElement.contains(ancestor)) { return; }
