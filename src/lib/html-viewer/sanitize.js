...
const SAFE_ID = Date.now()+'.'+Math.random().toString(36).substring(2);
...
export const ALLOW_ATTRS = ['contenteditable', 'data-safe-html', 'data-safe-id', 'data-cid', 'embedded-card', 'embedded-image', 'collapsed', 'emoji', 'embedded-link-card', 'button-toggle-shrink-image', 'button-remove-image', 'button-resize-card-options', 'button-remove-card'];
...
export default function sanitize(html, processors) {
	if (html in CACHE) return CACHE[html];
	let sanitized = doSanitize(html, processors);
	return CACHE[html] = sanitized;
}

const each = (arr, fn) => Array.prototype.slice.call(arr).forEach(fn);

export function doSanitize(html, processors, returnDocument) {
	let frag = dompurify.sanitize(html, SANITIZE_OPTIONS);
	...
	function sanitizeUrlAttribute(node, name) {
		if (node.hasAttribute(name) && !isWithinSafeRegion(node)) {
			let attr = node.getAttribute(name),
				sanitized = sanitizeUrl(attr, false, String(node.nodeName).toLowerCase()==='img' && name==='src');
	...
	function rm(attr, fn) {
		let all = doc.querySelectorAll('['+attr+']');
	...
	return returnDocument===true ? doc : doc.body.innerHTML;
}
...
function isWithinSafeRegion(node) {
	let c = node;
	while (c) {
		if (c.__safe===true || (c.getAttribute!=null && c.getAttribute('data-safe-html')===SAFE_ID)) {
 ...
 */
let CSS_PROPERTY_WHITELIST = /^\s*(background(-(image|position(-[xy])?|repeat(-[xy])?|size|color))?|display|position|left|right|top|bottom|(?:(min|max)-)?(height|width)|float|clear|(margin|padding)(-(left|top|right|bottom))?|border(-.+)?|box-shadow|list-style(-.+)?|color|vertical-align|line-height|white-space|font(-(variant|family|size|weight|style))?|text-(align|transform|decoration(-(style|color|line))?))\s*$/i;

function sanitizeUrl(url, ret, allowZimbra) {
	url = String(url==null ? '' : url).replace(/(^['"\s]+|['"\s]+$)/g, '');
	if (!url.match(/^(?:https?:\/\/|(?:mailto|tel|cid|data):)/) && (!allowZimbra || !url.match(/^(\/@zimbra)?\/service\/home\/~\/\?/))) {
...
function inlineStyles(doc) {
	...
	each(doc.getElementsByTagName('style'), style => {
		let rules = style.sheet && style.sheet.cssRules;
		for (let i=0; rules && i<rules.length; i++) {
			let rule = rules[i];
			if (rule.type!==1) continue;
			let key, value, css = '', s = rules[i].style;
			for (let j=0; j<s.length; j++) {
				key = s[j];
				if (CSS_PROPERTY_WHITELIST.test(key) && (value=s.getPropertyValue(key)) && value!=='initial') {
			...
			let sel = rules[i].selectorText.replace(/\b(:(hover|focus|active|visited)|::[a-z-]+)\b/gi, '');
