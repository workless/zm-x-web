...
function normalizeCid(cid) { return cid.replace(/[<>]/g, ''); }

function reverseNiceKey(name, type) {
	let map = (type && TYPED_MAPS[type]) || MESSAGE_FIELD_MAP;
	for (let i in map) {
...
function parseSenders(message) {
	...
	let mapping = { f: 'from', t: 'to', c: 'cc', b: 'bcc', s: 'sender' };
	for (let i = 0; i < message.senders.length; i++) {
		let sender = message.senders[i],
	...
	if (obj && type === 'object') {
		...
		for (let niceKey in obj) {
			if (obj.hasOwnProperty(niceKey)) {
				...
				out[i] = normalizeToZimbra(
					obj[niceKey],
					path.concat(niceKey.replace(/s$/, ''))
				);
			}
		}
		obj = out;
...
const CLASSIFIERS = [
	...
	},
	{
		...
		test: (obj, path) =>
			!Array.isArray(obj) &&
			path.lastIndexOf('contact') >= Math.max(path.length - 2, 0),
		process(contact) {
	...
	},
	{
		...
		test: (obj, path) =>
			...
			obj.hasOwnProperty('absFolderPath'),
		process(folder) {
...
function reduceMimeParts(obj, iterator, accumulator) {
	let parts = obj.mimeParts || (obj.mimePart && [obj.mimePart]);
...
export default function zimbraApiClient(config) {
		...
		if (obj && typeof obj === 'object') {
			...
			for (let i in obj) {
				if (obj.hasOwnProperty(i) && i !== '_jsns') {
					...
					// let niceKey = RESPONSE_FIELD_MAP[i] || i;
					out[niceKey] = normalizeZobj(
						obj[i],
						path.concat(depluralize(niceKey))
					);
				}
			}
			obj = denest(out, path);
	...
	function after(req, res) {
		 ...
		 let resSessionID = get(res, 'data.Header.context.session.id');
		...
		if (changes) {
			...
			changes = changes.forEach(({ seq }) => (typeof seq !== undefined && seq > (highestSeq || 0)) && (highestSeq = seq));
		}
		if (!outstandingRequests) {
			if (highestSeq) notifySeq = highestSeq;
			if (queuedChanges.length) {
				//sort queued changes and emit
				api.emit('change', normalizeZobj(queuedChanges.sort((a,b) => a.seq < b.seq ? -1 : a.seq > b.seq ? 1 : 0)));
				queuedChanges = [];
			}
		}
	...
	api.soapRequest = (type, body, options) => {
		...
		let url = '/service/soap/' + type || options.type;
