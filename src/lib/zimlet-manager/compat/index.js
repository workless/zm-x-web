...
function createApi(context) {
	...
	context.API = api;
	let props = { __zimletContext: context };
...
class ZmZimletBase {
	xmlObj(key) {
		let ctx = this.__zimletContext || {};
		return key!=null ? ctx[key] : ctx;
	}

	getMessage(key) {
		let properties = this.xmlObj('properties');
		if (properties) return properties[key];
	}

	getConfig(value) {
		let { config } = this.xmlObj();
	...
	getResource(filename) {
		let opts = this.xmlObj();
		if (opts.getResourceUrl) {
			let url = opts.getResourceUrl(filename);
...
class ZmMetaData {
	...
	get(key, value, successCallback, errorCallback) {
		let account = this.__zimletContext.getAccount();
		let ident = find(account && account.identities, 'id', this.id);
...
const AjxDispatcher = {
	...
	addPackageLoadFunction(...args) {
		let pkg = this.__zimletContext.API.AjxPackage.current;
...
const AjxTemplate = {
	...
	getTemplate(name) {
		let template = AjxTemplate.registry[name];
...
const AjxPackage = {
	...
	undefine(name) {
		if (this.current===name) {
			...
			let index = this.stack.indexOf(name);
			if (index!==-1) {
