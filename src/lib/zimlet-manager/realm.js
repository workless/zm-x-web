	...
	createContext({ name })
		.then( context => {
			let controller = /** @lends Context# */ {
				...
				expose(properties) {
					for (let i in properties) if (properties.hasOwnProperty(i)) {
...
*/
const createEvalForContext = context => (code, { wrap=true, sourceUrl }={}) => new Promise( (resolve, reject) => {
	let keys = Object.keys(context);
	let { document } = context;
	let wrappedCode = wrap ? wrapCode(code) : unwrappedCode(code);
	context.__context = {
		name,
		...
		result(result, error) {
			...
			let globals = getNewGlobals(context, keys);
...
const createContext = ({ name }) => new Promise( (resolve, reject) => {
	let frame = document.createElement('iframe');
	frame.name = name || 'Unnamed Context';

	// Frame must be at least 32px to allow generation of drag/drop badges.
	frame.style.cssText = 'position:absolute; left:0; top:-10000px; width:32px; height:32px; overflow:hidden;';
